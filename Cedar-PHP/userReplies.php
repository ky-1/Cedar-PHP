...
if ($user_result->num_rows == 0) {
    ...
    if ($user['hide_replies'] == 1 && $user['user_id'] != $_SESSION['user_id'] && (empty($_SESSION['signed_in']) || $is_admin['user_level'] == 0)) {
        exit();
    }

    if (!((isset($_GET['offset']) && is_numeric($_GET['offset'])) && isset($_GET['dateTime']))) {
        $tabTitle = htmlspecialchars($user['nickname'], ENT_QUOTES) .'\'s Replies - Cedar';
        ...
        while ($replies = $replies_result->fetch_array()) {
            ...
            echo '<div data-href="/replies/'. $replies['reply_id'] .'" class="post post-subtype-default trigger">
			  <p class="community-container">
			    <a class="test-community-link" href="/posts/'. $replies['reply_post'] .'"><img src="'. (isset($user_post['name_color']) ? printFace($user_post['user_face'], $reply_post['feeling_id'], $user_post['name_color']) : printFace($user_post['user_face'], $reply_post['feeling_id'])) .'" class="community-icon">Comment on '. htmlspecialchars($user_post['nickname'], ENT_QUOTES) .'\'s Post</a>
			  </p>
            ...
            echo ($reply_user['hide_online'] == 0 ? (strtotime($reply_user['last_online']) > time() - 35 ? ' online' : ' offline') : '') . '"><img src="'. (isset($reply_user['name_color']) ? printFace($reply_user['user_face'], $replies['feeling_id'], $reply_user['name_color']) : printFace($reply_user['user_face'], $replies['feeling_id'])) .'" id="icon"></a><p class="user-name"><a'. (isset($reply_user['name_color']) ? ' style="color: '. $reply_user['name_color'] .'"' : '') .' href="/users/'. $reply_user['user_name'] .'/posts">'. htmlspecialchars($reply_user['nickname'], ENT_QUOTES) .'</a></p><p class="timestamp-container"><a id="timestamp">' . humanTiming(strtotime($replies['date_time'])) . '</a></p><div id="body">';
