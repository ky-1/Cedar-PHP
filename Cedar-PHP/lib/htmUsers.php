      ...
      <div id="sidebar-profile-body" class="'.(isset($fav_post['post_image'])?'with-profile-post-image':'').'">
        <div class="icon-container'. ($user['user_level'] > 1 ? ' verified' : '') . ($user['hide_online'] == 0 ? (strtotime($user['last_online']) > time() - 35 ? ' online' : ' offline') : '') .'">
          <a href="/users/'.$user['user_name'] .'/posts">
            <img src="'. (isset($user['name_color']) ? printFace($user['user_face'], 0, $user['name_color']) : printFace($user['user_face'], 0)) .'" alt="'. htmlspecialchars($user['nickname'], ENT_QUOTES) .'" id="icon">
          </a>
        </div>
...
<li><a href="/users/'. $user['user_name'] .'/block" class="symbol user-menu-block"><span>Block User</span></a></li>
        ...
        if (!$friends_result->num_rows == 0) {
            ...
            <p class="description">Friend Request: <img width="36px" height="36px" src="'. (isset($user['name_color']) ? printFace($user['user_face'], 0, $user['name_color']) : printFace($user['user_face'], 0)) .'">'. htmlspecialchars($user['nickname'], ENT_QUOTES) .'</p>
...
function userInfo($user)
{
    ...
    ?><div class="ads">
        <span class="ads-label">ewweweww wtf ads on cedat? yeah.,ads give money so i can host the site. but you can <a href="#" class="noads">disable ads</a></span>
        <ins class="adsbygoogle"
            style="display:block"
            data-ad-client="ca-pub-2580338164616812"
            data-ad-slot="5573461353"
