function bindEvents() {
	$('#delete_user').off().on('submit', function(e){
		...
		user_reason = $('textarea[name="user_reason"]').val();
		csrfToken = $('input[name="csrfToken"]').val();
		
		$.ajax({
			type: 'POST',
			url: '/admin_panel/delete_user',
			data: {user_id:user_id, user_reason:user_reason, csrfToken:csrfToken}, 
	...
	$('#delete_post').off().on('submit', function(e){
		...
		$.ajax({
			...
			data: {post_id:post_id, post_violation_type:post_violation_type, post_reason:post_reason, csrfToken:csrfToken},
			success: function(data) {
				$('#delete-message').remove();
				if(data.success == 1){
	...
	$('#delete_reply').off().on('submit', function(e){
		...
		$.ajax({
			...
			data: {reply_id:reply_id, reply_violation_type:reply_violation_type, reply_reason:reply_reason, csrfToken:csrfToken},
			success: function(data) {
				$('#delete-message').remove();
				if(data.success == 1){
