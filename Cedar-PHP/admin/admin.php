        ...
        if (isset($action)) {
            if ($action == 'delete_user') {
                ...
                if ($user_result->num_rows == 0) {
                    ...
                    } else if ($_POST['user_reason'] == '') {
                        exit('{"success":0,"problem":"Please specify a reason for deletion."}');
                    } else if ($_POST['csrfToken'] != $_SESSION['csrfToken']) {
                        exit('{"success":0,"problem":"CSRF token check failed."}');
                    } else {
                        $delete_user = $dbc->prepare('DELETE FROM users WHERE user_id = ? LIMIT 1');
                        $delete_user->bind_param('i', $user['user_id']);
                        $delete_user->execute();

                        $json = '{"embeds": [{"color": "1215778","thumbnail": {"url": "'. printFace($user['user_face'], 0) .'"},"title": "Deleted user.","description": "Deleted user: '. $user['user_name'] .'\nDeleted By: '. $_SESSION['user_name'] .'\nReason for deletion: \"'. $_POST['user_reason'] .'\"","footer": {"text": "Date Deleted: '. date("m/d/Y g:i A") .'"}}]}';

                        $ch = curl_init('https://discordapp.com/api/webhooks/414947465309978624/SaqDIbpMBLlJ0188NAmqGWdKFPYFSCAYAZFV7EvZF-H7l6nw9jlzxavpk9d03SsroUwe');
                ...
                $get_post = $dbc->prepare('SELECT * FROM posts INNER JOIN users ON user_id = post_by_id WHERE id = ? LIMIT 1');
                ...
                if ($post_result->num_rows == 0) {
                    ...
                    $json = '{"embeds": [{"color": "1215778","thumbnail": {"url": "'. printFace($post['user_face'], 0) .'"},"title": "Deleted post.","description": "Post Text: \"'. $post['text'] .'\"\nPost By: '. $post['user_name'] .'\nDeleted By: '. $_SESSION['user_name'] .'\nReason for deletion: \"'. $_POST['post_reason'] .'\""'. (!empty($post['post_image']) ? ',"image": {"url": "'. $post['post_image'] .'"}' : '') .',"footer": {"text": "Date Deleted: '. date("m/d/Y g:i A") .'"}}]}';
                ...
                $get_reply = $dbc->prepare('SELECT * FROM replies INNER JOIN users ON user_id = reply_by_id WHERE reply_id = ? LIMIT 1');
                ...
                if ($reply_result->num_rows == 0) {
                    ...
                    $json = '{"embeds": [{"color": "1215778","thumbnail": {"url": "'. printFace($reply['user_face'], 0) .'"},"title": "Deleted post.","description": "Post Text: \"'. $reply['text'] .'\"\nPost By: '. $reply['user_name'] .'\nDeleted By: '. $_SESSION['user_name'] .'\nReason for deletion: \"'. $_POST['reply_reason'] .'\""'. (!empty($reply['reply_image']) ? ',"image": {"url": "'. $reply['reply_image'] .'"}' : '') .',"footer": {"text": "Date Deleted: '. date("m/d/Y g:i A") .'"}}]}';
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form id="delete_user" action="/admin_panel/delete_user">
                                ...
                                <textarea class="form-control mt-2" name="user_reason" placeholder="Reason for deletion." rows="3"></textarea>
                                <input type="submit" class="btn btn-danger mt-2 float-right" value="Delete">
                            </form>
                        </div>
                    </div>
                </div>
