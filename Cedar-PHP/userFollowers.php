...
if ($user_result->num_rows == 0) {
    ...
    if (isset($_GET['offset']) && is_numeric($_GET['offset'])) {
        ...
        $get_followers = $dbc->prepare('SELECT * FROM follows WHERE follow_to = ? ORDER BY follow_id DESC LIMIT 20 OFFSET ?');
        $get_followers->bind_param('ii', $user['user_id'], $offset);
    } else {
        $tabTitle = $user['nickname'] .'\'s Followers - Cedar';
		...
		<div class="main-column"><div class="post-list-outline">
		  ...
		  <div class="list follow-list following">
		    <ul class="list-content-with-icon-and-text arrow-list" id="friend-list-content" data-next-page-url="/users/'. $user['user_name'] .'/followers?offset=1&">';

        $get_followers = $dbc->prepare('SELECT * FROM follows WHERE follow_to = ? ORDER BY follow_id DESC LIMIT 20');
    ...
    if (!$followers_result->num_rows == 0) {
        while ($followers = $followers_result->fetch_array()) {
            ...
            echo '<li class="trigger" data-href="/users/'. $follow_user['user_name'] .'/posts">
				  <a href="/users/'. $follow_user['user_name'] .'/posts" class="icon-container'.($follow_user['user_level'] > 1 ? ' verified' : '').'">
				  ...
				  <div class="body">
				    <p class="title">
				      <span class="nick-name">
				        <a '. (isset($follow_user['name_color']) ? 'style="color: '. $follow_user['name_color'] .'"' : '') .' href="/users/'. $follow_user['user_name'] .'/posts">'. $follow_user['nickname'] .'</a>
				      </span>
