...
if (!isset($_GET['query'])) {
    ...
    if (strlen($_GET['query']) < 1 || strlen($_GET['query']) > 16) {
        ...
        if (isset($_GET['offset']) && is_numeric($_GET['offset'])) {
            ...
            $get_searched_users = $dbc->prepare('SELECT * FROM users INNER JOIN profiles ON profiles.user_id = users.user_id WHERE user_name COLLATE UTF8MB4_GENERAL_CI LIKE ? ESCAPE "/" OR nickname LIKE ? ESCAPE "/" ORDER BY user_name DESC LIMIT 50 OFFSET ?');
            ...
            $get_searched_users = $dbc->prepare('SELECT * FROM users INNER JOIN profiles ON profiles.user_id = users.user_id WHERE user_name COLLATE UTF8MB4_GENERAL_CI LIKE ? ESCAPE "/" OR nickname LIKE ? ESCAPE "/" ORDER BY user_name DESC LIMIT 50');
