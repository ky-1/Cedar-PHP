...
function printHeader($on_page)
{
    ...
    if (isset($_SERVER['HTTP_X_PJAX'])) {
        if (isset($tabTitle)) {
            echo '<title>'. htmlspecialchars($tabTitle, ENT_QUOTES) .'</title>
            <script>
    ...
    if (isset($_COOKIE['cedar_color_theme'])) {
        ...
        .list .toggle-button .follow-done-button:before, #image-header-content .image-header-title .title, .user-organization, .user-sidebar .user-dropdown-button:before, #user-dropdown-menu .symbol:before, .list .toggle-button .follow-button:before, .create-button:before, div#activity-feed-tutorial h3, .user-sidebar .friend-button:before, .user-sidebar .follow-button:before {color: hsl('.$HSL[0].','.$HSL[1].'%,'.$HSL[2].'%);}
    ...
    <script>
        (adsbygoogle = window.adsbygoogle || []).push({
            google_ad_client: "ca-pub-2580338164616812",
        ...
        <div id="sub-body">
          <menu id="global-menu">
            <li id="global-menu-logo">
                <h1><a href="/"><img src="https://web.archive.org/web/20180627233737im_/https://cedar.doctor/assets/img/cedar-logo.png" alt="Miiverse" width="120" height="30"></a></h1></li>
    <?php
    ...
    if (!empty($_SESSION['signed_in'])) {
        ...
        <li id="global-menu-list">
            <ul>
                ...
                <li id="global-menu-message"<?= ($on_page == 4 ? ' class="selected"' : '') ?>>
                    <a href="/messages" class="symbol"><span>Messages</span><span class="badge" style="display: none;">0</span></a>
                </li>
                <li id="global-menu-news"<?= ($on_page == 5 ? ' class="selected"' : '') ?>>
                    <a href="/notifications" class="symbol"><span class="badge" style="display: none;">0</span></a>
                </li>
                <li id="global-menu-my-menu"><button class="symbol js-open-global-my-menu open-global-my-menu"></button>
                    <menu id="global-my-menu" class="invisible none">
                        ...
                        <li><a href="/rules" class="symbol my-menu-guide"><span>Cedar Rules</span></a></li>
                        <?= ($user['user_level'] > 0 ? '<li><a href="/admin_panel" class="symbol my-menu-miiverse-setting"><span>Admin Panel</span></a></li>' : '') ?>
                        <li><a href="/blocked" class="symbol my-menu-blocked"><span>Blocked Users</span></a></li>
                        <li>
...
function printPost($post, $reply_pre)
{
    ...
    if (!empty($video_id[1])) {
        echo '<a href="/posts/'. $post['id'] .'" class="screenshot-container video"><img height="48" src="https://i.ytimg.com/vi/'. $video_id[1] .'/default.jpg"></a>';
    }
...
function uploadImage($img, $width, $height)
{
    ...
    $pvars = array('file' => (exif_imagetype($filename) == 1 ? 'data:image/gif;base64,' : (exif_imagetype($filename) == 2 ? 'data:image/jpg;base64,' : (exif_imagetype($filename) == 3 ? 'data:image/png;base64,' : (exif_imagetype($filename) == 6 ? 'data:image/bmp;base64,' : (substr($img['name'], -4) == '.mp3' ? 'data:audio/mpeg;base64,' : (substr($img['name'], -4) == '.ogg' ? 'data:audio/ogg;base64,' : (substr($img['name'], -5) == '.webm' ? 'data:video/webm;base64,' : ''))))))) . (isset($resized) ? base64_encode($resized) : base64_encode($data)),
