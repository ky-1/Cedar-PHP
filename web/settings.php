...
if (empty($_SESSION['signed_in'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] != 'POST') {
        ...
        $tabTitle = 'Profile Settings - Cedar';
          ...
          <ul class="settings-list">
            ...
            <li>
              <div style="text-align: center;">
                ...
                <input name="face-type" type="radio" value="1" '. (strpos($user['user_face'], "imgur") || strpos($user['user_face'], "cloudinary") ? 'checked' : '') .' style="margin-left: 5px; display: inline; margin-right: 50px; margin-top: 20px;">
                <p style="display: inline;">Mii:</p>
                <input name="face-type" type="radio" value="2" '. (strpos($user['user_face'], "imgur") || strpos($user['user_face'], "cloudinary") ? '' : 'checked') .' style="margin-left: 5px; display: inline;">
              </div>
              <div class="custom-face'. (strpos($user['user_face'], "imgur") || strpos($user['user_face'], "cloudinary") ? '' : ' none') .'">
                <p class="settings-label" style="margin: 0 0 10px;">Profile picture upload</p>
                <input type="file" name="face" accept="image/*">
              </div>
              <div class="nnid-face'. (strpos($user['user_face'], "imgur") || strpos($user['user_face'], "cloudinary") ? ' none' : '') .'">
                <p class="settings-label" style="margin: 0 0 10px;">NNID</p>
            ...
            <li>
                <p class="settings-label">
                    <label>Do you want to hide your online status?</label>
                </p>
                <div class="select-content">
                    <div class="select-button">
                        <select name="hide_online" id="yeah_notifs">
                            <option value="0"'. ($user['hide_online'] == 0 ? ' selected' : '') .'>Don\'t Hide</option>
                            <option value="1"'. ($user['hide_online'] == 1 ? ' selected' : '') .'>Hide</option>
                        </select>
                    </div>
                </div>
            </li>
            ...
            <li>
                <p class="settings-label">
                    <label>Do you want to hide your replies page?</label>
                </p>
                <div class="select-content">
                    <div class="select-button">
                        <select name="hide_replies" id="yeah_notifs">
                            <option value="0"'. ($user['hide_replies'] == 0 ? ' selected' : '') .'>Don\'t Hide</option>
                            <option value="1"'. ($user['hide_replies'] == 1 ? ' selected' : '') .'>Hide</option>
                        </select>
                    </div>
                </div>
            </li>
            ...
            <li>
                <p class="settings-label">
                    <label>Do you want to hide your Yeahs page?</label>
                </p>
                <div class="select-content">
                    <div class="select-button">
                        <select name="hide_yeahs" id="yeah_notifs">
                            <option value="0"'. ($user['hide_yeahs'] == 0 ? ' selected' : '') .'>Don\'t Hide</option>
                            <option value="1"'. ($user['hide_yeahs'] == 1 ? ' selected' : '') .'>Hide</option>
                        </select>
                    </div>
                </div>
            </li>
...
<input type="submit" name="submit" class="black-button apply-button" value="Save Settings" /></div></form></div></div></div></div>';
        ...
        if (isset($_POST['name-color']) && $_POST['name-color'] !== '#ffffff') {
            $user_change = $dbc->prepare('UPDATE users SET name_color = ? WHERE user_id = ?');
            $user_change->bind_param('si', $_POST['name-color'], $_SESSION['user_id']);
            $user_change->execute();
        } else {
            $user_change = $dbc->prepare('UPDATE users SET name_color = NULL WHERE user_id = ?');
            $user_change->bind_param('i', $_SESSION['user_id']);
            $user_change->execute();
        }

        $user_change = $dbc->prepare('UPDATE users SET hide_online = ? WHERE user_id = ?');
        $user_change->bind_param('ii', $_POST['hide_online'], $_SESSION['user_id']);
        $user_change->execute();

        $user_change = $dbc->prepare('UPDATE profiles SET hide_replies = ? WHERE user_id = ?');
        $user_change->bind_param('ii', $_POST['hide_replies'], $_SESSION['user_id']);
        $user_change->execute();

        if (isset($_POST['face'])) {
            if ($_POST['face-type'] == 2 && !empty($_POST['face'])) {
                $ch = curl_init();
                ...
                curl_setopt($ch, CURLOPT_URL, $api . "admin/mapped_ids?input_type=user_id&output_type=pid&input=" . $_POST['face']);
                ...
                foreach (json_decode(json_encode($miis), true)["mii"]["images"]["image"] as $a) {
                    if ($a["type"] == "normal_face") {
                        $face_id = str_replace('_normal_face.png', '', str_replace('https://mii-images.cdn.nintendo.net/', '', str_replace('http://mii-images.cdn.nintendo.net/', '', $a["cached_url"])));
                    }
                }
